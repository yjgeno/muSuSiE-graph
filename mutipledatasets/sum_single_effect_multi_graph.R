...
source("mutipledatasets/multi_utility.R")
sum_single_effect_multi <- function(dta_list, scale_x = TRUE, intercept = TRUE,
  ...
  for (iter_K in seq_len(K)) {
    ...
    beta_list[[iter_K]] <- rowSums(b_list[[iter_K]])
    ...
    dta_list[[iter_K]]$XtX <- crossprod(dta_list[[iter_K]]$X_scale)
    ...
    dta_list[[iter_K]]$XtY <- crossprod(dta_list[[iter_K]]$X_scale, dta_list[[iter_K]]$Y)
  }
  ...
  for (iter in seq_len(itermax)) {
    ...
    for (l in seq_len(L)) {
      # update parameters
      for (iter_K in seq_len(K)) {
        beta_use[[iter_K]] <- beta_list[[iter_K]] - b_list[[iter_K]][, l]
        XtXbeta_use[[iter_K]] <- dta_list[[iter_K]]$XtX %*% beta_use[[iter_K]]
        XtYtmp <- dta_list[[iter_K]]$XtY - XtXbeta_use[[iter_K]]
        b_hat_mat[, iter_K] <- XtYtmp / dta_list[[iter_K]]$X2
      ...
      for (iter_K in seq_len(K)) {
        ...
        b2_list[[iter_K]][, l] <- alpha_list[[iter_K]][, l] *
          (post_mu_mat[, iter_K]^2 + post_sigma2_mat[, iter_K])
      }
      ## Calculate the KL-divergence
      KL_div <- KL_div + KL_fun_multi_graph(dta_list, XtXbeta_use, sigma2, b_list, b2_list, lBF_model, K, l)
      for (iter_K in seq_len(K)) {
        beta_list[[iter_K]] <- beta_use[[iter_K]] + b_list[[iter_K]][, l]
      }
    }
    ERSS <- 0
    for (iter_K in seq_len(K)) {
  ...
  if (length(index_L) > 0) {
    ...
    for (iter_K in seq_len(K)) {
      ...
      out_res$res[[iter_K]]$Xb <- dta_list[[iter_K]]$mean_Y +
        dta_list[[iter_K]]$X_scale %*% out_res$res[[iter_K]]$post_mean
    }
  } else {
