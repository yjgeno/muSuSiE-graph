---
title: "muSuSiE-graph-two"
output: html_document
date: "`r Sys.Date()`"
---

## Generate random data

```{r}
## define parameters
p <- 100
n1 <- 300
n2 <- 400
p_c <- 100
p_1 <- 30
p_2 <- 25
sigma <- 1
sigma0 <- 0.6
A1 <- matrix(0, nrow = p, ncol = p)
A2 <- matrix(0, nrow = p, ncol = p)
set.seed(2021)
# Define the true graph given order
index_c <- sample(seq_len(p * (p - 1) / 2), size = p_c, replace = FALSE)
index_1 <- sample(setdiff(seq_len(p * (p - 1) / 2), index_c), size = p_1, replace = FALSE)
index_2 <- sample(setdiff(seq_len(p * (p - 1) / 2), index_c), size = p_2, replace = FALSE)

A1[lower.tri(A1)][c(index_c, index_1)] <-  rnorm(p_c + p_1, mean = 0, sd = sigma0)
A2[lower.tri(A2)][c(index_c, index_2)] <-  rnorm(p_c + p_2, mean = 0, sd = sigma0)

alpha_mat_1 <- matrix(0, nrow = p, ncol = p)
alpha_mat_1[lower.tri(alpha_mat_1)][c(index_c, index_1)] <- 1
alpha_mat_2 <- matrix(0, nrow = p, ncol = p)
alpha_mat_2[lower.tri(alpha_mat_2)][c(index_c, index_2)] <- 1

eps_1 <- matrix(rnorm(p * n1), nrow = p, ncol = n1)
dta_1 <- solve(diag(1, nrow = p) - A1, eps_1)
dta_1 <- t(dta_1)
eps_2 <- matrix(rnorm(p * n2), nrow = p, ncol = n2)
dta_2 <- solve(diag(1, nrow = p) - A2, eps_2)
dta_2 <- t(dta_2)
```

## Get graph

For two data sets, we can use function **joint_graph_fun_two** in **muSuSiE-graph-two.R**. Here

1. dta_1 is n_1 x p, where each row is a sample, each column is a feature.

2. dta_2 is n_2 x p. Two data sets can have different sample size but must have the same features.

3. scale_x: scale the data.

4. intercept: center data such that the mean is 0.

5. **prior_vec:** you can change this to make result better :).

6. itermax: the maximum iterations to run muSuSiE.

7. **L_max:** maximum number of edges for one node; too small is not stable; too large will be time-consuming.

```{r}
source("twodatasets/muSuSiE-graph-two.R")
out_res <- joint_graph_fun_two(dta_1, dta_2, scale_x = FALSE, intercept = TRUE,
                               sigma02_int = NULL, sigma2_int = NULL, prior_vec = NULL,
                               itermax = 100, L_max = 10, tol = 1e-4, sigma0_low_bd = 1e-8,
                               residual_variance_lowerbound = NULL)
```

## Check results

```{r}
names(out_res)
```

1. alpha_res_1: a p x p matrix, the (ij)-th entry is the probability that the edge i -> j exist.

2. A_res_1: a p x p matrix, the regression coefficients.
